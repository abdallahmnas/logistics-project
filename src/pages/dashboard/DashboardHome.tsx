...
export const DashboardHome: React.FC = () => {
  ...
  const { requests: procurements } = useAppSelector(
  ...
  const consolidationItems =
    packages.filter(
      (p) =>
        p.status === "received_at_warehouse" ||
        p.status === "at_china_warehouse",
    ).length || 2;
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative z-10">
          <div>
            ...
            <p className="text-blue-200 text-sm m-0 leading-relaxed">
              ...
              <span className="text-brand-orange font-bold">
                {activePackages.length} active shipments
              </span>{" "}
              in transit and{" "}
              <span className="text-brand-orange font-bold">
          ...
          <div className="flex gap-4">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl px-6 py-4 text-center border border-white/10 min-w-[120px]">
              ...
              <div className="text-[10px] font-bold text-blue-300 uppercase tracking-wider mt-1">
                Active
                <br />
                Shipments
              </div>
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm">
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100">
              ...
              <Link
                ...
                className="text-sm font-bold text-[#0A1128] hover:text-brand-orange no-underline flex items-center gap-1"
              >
                View All ➔
              </Link>
            </div>
                ...
                <tbody>
                  {recentPackages.map((pkg) => (
                    <tr
                      ...
                      onClick={() => navigate(`/customer/shipments/${pkg.id}`)}
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${pkg.shippingMethod === "sea" ? "bg-blue-100 text-blue-600" : "bg-orange-100 text-orange-600"}`}
                          >
                            {pkg.shippingMethod === "sea" ? (
                              <InboxOutlined />
                            ) : (
                              <CarOutlined />
                            )}
                      ...
                      <td className="px-4 py-4">
                        <div className="text-xs text-slate-600">
                          {pkg.originCountry || "China"}{" "}
                          <span className="text-slate-400 mx-1">→</span>{" "}
                          {pkg.destinationCity || "Lagos"}
                        </div>
                      </td>
                      <td className="px-4 py-4">
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden relative">
            <div className="absolute top-4 left-4 z-10 bg-white/95 backdrop-blur-sm rounded-lg p-3 shadow-sm border border-slate-100">
              ...
              <div className="flex gap-4 text-[10px] text-slate-600">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-[#0A1128]"></span> US
                  Hub: Nominal
                </span>
              </div>
            </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
            ...
            <p className="text-slate-500 text-xs leading-relaxed m-0 mb-4">
              You have {consolidationItems} packages waiting in the US
              warehouse. Consolidate them now to save up to 40% on shipping.
            </p>
            <Button
              block
              size="large"
            ...
            <div className="space-y-4">
              {recentTransactions.map((tx) => (
                <div key={tx.id} className="flex items-center gap-3">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${tx.type === "credit" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
                  >
                    {tx.type === "credit" ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-slate-800 truncate">
                      {tx.description ||
                        (tx.type === "credit" ? "Wallet Top-up" : "Payment")}
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    <div
                      className={`text-sm font-bold ${tx.type === "credit" ? "text-green-600" : "text-slate-800"}`}
                    >
                      {tx.type === "credit" ? "+" : "-"}
                    </div>
          ...
          <div className="bg-gradient-to-br from-brand-orange to-[#E86E21] rounded-xl p-6 text-white shadow-lg relative overflow-hidden">
            ...
            <div className="text-center relative z-10 mb-4">
              ...
              <p className="text-orange-100 text-xs m-0 leading-relaxed">
                You're 250 points away from Platinum Tier. Earn points on every
                shipment.
              </p>
            </div>
            <Button
              block
              size="large"
              className="bg-white hover:bg-slate-50 text-[#0A1128] border-none font-bold shadow-sm mb-3"
            >
              Redeem Rewards
            </Button>
            <div className="text-center">
              <Button
                type="link"
                className="text-white/80 hover:text-white text-xs font-medium p-0"
              >
