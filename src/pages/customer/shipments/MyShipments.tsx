  ...
  const columns = [
      ...
      render: (_: unknown, record: Package) => (
        <div>
          ...
          {record.chineseTrackingNo && (
            <div className="text-xs text-slate-500">CN: {record.chineseTrackingNo}</div>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (method: string) => (
        <Tag color={method === 'air' ? 'blue' : method === 'sea' ? 'cyan' : 'default'} className="uppercase">
          {method || 'Pending'}
        </Tag>
      ),
    },
    ...
    },
    {
      ...
      render: (_: unknown, record: Package) => (
        <Button
          ...
          onClick={() => navigate(`/dashboard/shipments/${record.id}`)}
        >
          View
        </Button>
      ),
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 m-0">My Shipments</h1>
          <p className="text-slate-500 mt-1 mb-0 text-sm">Track and manage your packages</p>
        </div>
        <div className="flex gap-2">
          <Button
            size="large"
            className="font-bold"
            onClick={() => navigate('/dashboard/shipments/consolidate')}
          >
            Consolidate Items
          </Button>
          <Button
            ...
            onClick={() => navigate('/dashboard/shipments/pre-alert')}
          >
            Pre-Alert Package
          </Button>
        </div>
      ...
      <Card bordered={false} className="shadow-sm rounded-2xl">
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          />
          <Select defaultValue="all" size="large" className="md:w-48" onChange={setFilterStatus}>
          ...
          items={[
            ...
            },
            {
              ...
              children: (
                <Table
                  columns={columns}
                  dataSource={filteredPackages.filter((p) => p.paymentStatus === 'unpaid' && p.invoiceAmount)}
                  rowKey="id"
                  loading={loading}
                  scroll={{ x: 800 }}
                  locale={{ emptyText: 'No packages need attention at the moment.' }}
                />
              ),
            },
          ]}
